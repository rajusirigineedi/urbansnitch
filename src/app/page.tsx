...
export default function Home() {
  ...
  const reviewCardVariants = {
    ...
    visible: { opacity: 1, x: 0, rotateY: 0, transition: { duration: 0.8 } },
          ...
          {isDesktop &&
            colorCardData.map((item) => (
              <Link href={item.href} key={item.data.title}>
                <motion.div
                  ref={landingPageColorCardRef}
                  variants={landingPageColorCardVariants}
                  initial="hidden"
                  animate={landingPageColorCardControls}
                >
          ...
          <div className="flex flex-wrap justify-center items-center gap-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2 mt-6">
              {landingPageProductData.map((data, index) => (
                <motion.div
                  ref={landingPageProductRef}
                  variants={landingPageProductCardVariants}
                  initial="hidden"
                  animate={landingProductControls}
                  key={index}
                >
          ...
          {isDesktop &&
            reviewData.map((item, index) => (
              <motion.div
                ...
                key={index}
              >
                <ReviewCard data={item} key={item.title} />
              </motion.div>
            ))}
          {!isDesktop &&
            reviewData.map((item, index) => (
